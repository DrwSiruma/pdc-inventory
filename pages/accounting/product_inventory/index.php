    ...
    <div class="container-fluid">
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-0">Product Inventory Monitoring</h5>
                    <small class="text-muted">Daily outlet inventory submitted by stores.</small>
                </div>
                <div>
                    <a href="daily.php" class="btn btn-primary"><i class="bi bi-calendar-event"></i>&nbsp;Daily Monitor</a>
                    <a href="history.php" class="btn btn-secondary"><i class="bi bi-clock-history"></i>&nbsp;History</a>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table id="productInventoryTable" class="table table-bordered table-hover align-middle">
                        <thead>
                            <tr>
                                <th>Business Date</th>
                                <th>Store</th>
                                ...
                                <?php while($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            ?>
                                            <span class="badge bg-<?= $badge ?>"><?= ucfirst($row['business_status']); ?></span>
                                        </td>
                                        <td><?= $row['verified_by'] ?? '-'; ?></td>
                                        <td class="text-center">
                                            <a href="view.php?id=<?= $row['inventory_header_id']; ?>" class="btn btn-sm btn-info" title="View"><i class="bi bi-eye-fill"></i></a>
                                            <?php if($row['business_status']=='submitted'): ?>
                                            <a href="verify.php?id=<?= $row['inventory_header_id']; ?>" class="btn btn-sm btn-success" title="Verify"><i class="bi bi-check-circle-fill"></i></a>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="7" class="text-center text-muted">No submitted inventory found.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
